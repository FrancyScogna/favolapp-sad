...
function SearchAndSelect({ type = 'tutor', open, setOpen, setArray, array }) {
  ...
  const onClickSearch = async () => {
    ...
    try {
      if (searchField) {
        ...
        if (data.data.searchAllUsers.items.length > 0) {
          setResultArray([...data.data.searchAllUsers.items]);
          setNextToken(data.data.searchAllUsers.nextToken);
        } else {
  ...
  const onClickSearchMore = async () => {
    try {
      if (nextToken) {
        ...
        setResultArray((prev) => [...prev, ...data.data.searchAllUsers.items]);
        setNextToken(data.data.searchAllUsers.nextToken);
        setLoadingSearch(false);
      }
    } catch (error) {
  ...
  const onClickSelect = (id) => {
    const thereIsTutor = array.find((item) => item.id === id);
    if (!thereIsTutor) {
    ...
    <Dialog className='searchandselect-dialog' open={open}>
      <DialogTitle className='dialog-title'>
        <Typography className='title'>
          Cerca e seleziona un {type === 'tutor' ? 'tutor' : 'paziente'}
        </Typography>
