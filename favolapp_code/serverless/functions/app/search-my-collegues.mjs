...
export const handler = async (event, context, callback) => {
 try{
    ...
    let myColleguesUnfetched = [];
    let myColleguesFetched = [];
    const myColleguesFtch = await processCollegues(myCollegues);
    if(myColleguesFtch.length > 0){
     myColleguesFetched = myColleguesFtch.filter((item) => (item.name.includes(query) || item.surname.includes(query) || item.codfis.includes(query)));
     myColleguesUnfetched = myCollegues.filter(item1 => 
      myColleguesFetched.some(item2 => item1.userId.S === item2.id));
    }else{
     callback(null, {items: [], nextToken: null});
    }

    if(myColleguesFetched.length === 0){
     callback(null, {items: [], nextToken: null});
    }
